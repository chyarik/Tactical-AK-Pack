...
/**
Preparation for throwing grenade (used before throwing the grenade, when player is holding button)
*/
ACTION(ThrowPrepare)
...
/**
Used by player and AI whenever person is unconscious.
*/
ACTION(Unconscious)

/**
Actions for different kinds of injuries
*/
ACTION(UnconsciousHead)
ACTION(UnconsciousBody)
ACTION(UnconsciousArms)
ACTION(UnconsciousLegs)
...
/**
Used by player and AI. Animation used by medic when healing itself. Animation is ended by ACTION(Up).
*/
ACTION(Medic)
/**
Used by player and AI. Animation used by medic when healing somebody else. Animation is ended by ACTION(Up).
*/
ACTION(MedicOther)
/**
...
/**
Used by player and AI. Animation used during freefall
*/
ACTION(HaloFreeFall_non)
ACTION(HaloFreeFall_L)
ACTION(HaloFreeFall_R)
ACTION(HaloFreeFall_F)
ACTION(HaloFreeFall_B)

/**
Used by player and AI. Animation used during flying in parachute
*/
ACTION(para_pilot)
...
//for activation of freefall
ACTION(StartFreefall)


/**
Used when player's diary is to be shown (by UAShowMap action)
*/
ACTION(Diary)

/**
Used when player's Gear should be shown
*/
ACTION(Gear)
...
/**
Get over fences/obstacle action
*/
ACTION(GetOver)
...
//@{
/**
Used by player  for adjusting weapons
*/
ACTION(AdjustF)
ACTION(AdjustLF)
ACTION(AdjustRF)
...
/**
Used to influence all actions
*/
ACTION(All)

/**
Used by player and AI for the tactical movement. For player based on used input.
*/
ACTION(TactF)  // front
ACTION(TactLF) // front left
ACTION(TactRF) // front right
ACTION(TactL)  // left
ACTION(TactR)  // right
ACTION(TactLB) // back left
ACTION(TactRB) // back right
ACTION(TactB)  // back

/**
Used by player and AI for vertical swimming. For player based on used input.
*/
ACTION(SwimUp)
ACTION(SwimDown)

// "Player" moves, hopefully a temporary override
...
/*
Obstructed action for blocking the shooting in FFV
*/
ACTION(Obstructed)
